...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.patch_embed = PatchEmbed_vit_base(
...
class Block_vit_base(nn.Module):
...
class Attention_vit_base(nn.Module):
...
class PatchEmbed_vit_base(nn.Module):
    """
...
@BACKBONE_REGISTRY.register()
class D2BaseViT(ImageEncoderViT, Backbone):
    def __init__(self, cfg, input_shape):
        super().__init__(depth=12,
            embed_dim=768,
            img_size=384, # bryce 384 -> 1024 for GEM Base
            mlp_ratio=4,
