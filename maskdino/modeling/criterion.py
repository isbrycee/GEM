...
class SetCriterion(nn.Module):
    ...
    def loss_labels_ce(self, outputs, targets, indices, num_masks):
        ...
        loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight)
        # loss_ce = F.binary_cross_entropy_with_logits(src_logits.transpose(1, 2), target_classes.unsqueeze(1).float(), self.empty_weight)
    ...
    ######################################
    def loss_proposals(self, output_proposals, targets):
        ...
        bs = output_proposals["proposal_cls_logits"].shape[0]
        tgt_mask_list = []
        for b in range(bs):
            tgt_mask = targets[b]["masks"]
            tgt_mask_label = (~tgt_mask[0:1, :, :])
            # import cv2
            # import numpy as np
            # cv2.imwrite("test.jpg", (tgt_mask_label.squeeze().int().detach().cpu().numpy() * 255).astype(np.uint8))
            tgt_mask_list.append(tgt_mask_label)
        tgt_mask = torch.stack(tgt_mask_list).to(proposal_cls_logits)
        target_classes = F.adaptive_avg_pool2d(tgt_mask, output_size=proposal_size).flatten(2).squeeze(1).to(torch.int64)

        loss_proposal = F.cross_entropy(proposal_cls_logits, target_classes, ignore_index=-1)
        # loss_proposal = self.focal_loss(proposal_cls_logits, target_classes)
    ...
    #     target_classes = (self.num_classes - 1) * torch.ones([proposal_cls_logits.shape[0],
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        if outputs.get("proposal_cls_logits") is not None:
            ...
            proposal_loss_dict = self.loss_proposals(output_proposals, targets)
        ################################
