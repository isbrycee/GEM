...
from .biformer import Fake_FPN, Fake_FPN_with_BiFormer
from .cross_scale_fusion import HybridEncoder
...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.nhead = nhead
        self.num_encoder_layers = num_encoder_layers # add by hj
        if self.num_encoder_layers > 0:
            encoder_layer = MSDeformAttnTransformerEncoderLayer(d_model, dim_feedforward,
                                                                dropout, activation,
                                                                num_feature_levels, nhead, enc_n_points)  # add by hj
        else:
            encoder_layer = nn.Identity()
        self.encoder = MSDeformAttnTransformerEncoder(encoder_layer, num_encoder_layers)
        
#         encoder_layer = MSDeformAttnTransformerEncoderLayer(d_model, dim_feedforward,
#                                                             dropout, activation,
#                                                             3, nhead, enc_n_points) # add by hj
...
#         self.encoder = MSDeformAttnTransformerEncoder(encoder_layer, num_encoder_layers)
    ...
    def forward(self, srcs, masks, pos_embeds):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            if self.num_encoder_layers > 0: # add by hj
...
class MSDeformAttnTransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        ###########
#         reference_points_list = []
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device) # ([bs, 3024, 3, 2])
        
        ########### changed by hj
#         import pdb; pdb.set_trace()
#         for lvl, spatial_shape in enumerate(spatial_shapes):
#             reference_points_list.append(self.get_reference_points(spatial_shape.unsqueeze(0), valid_ratios[:,lvl,:].unsqueeze(1), device=src.device)) # ([bs, 3024, 3, 2])
        
#         level_start_index_list = []
#         padding_mask_list = []
        
#         split_size_or_sections = [None] * len(level_start_index) # [None, None, None]
#         for i in range(len(level_start_index)):
#             if i < len(level_start_index) - 1:
#                 split_size_or_sections[i] = level_start_index[i + 1] - level_start_index[i]
#             else:
#                 split_size_or_sections[i] = src.shape[1] - level_start_index[i] # [tensor(256, device='cuda:0'), tensor(1024, device='cuda:0'), tensor(4096, device='cuda:0')]
#         query_list = torch.split(src, split_size_or_sections, dim=1) # tuple ([2, 256, 256], [2, 1024, 256], [2, 4096, 256])
#         query_list = list(query_list)
#         pos_list = torch.split(pos, split_size_or_sections, dim=1)
#         padding_mask_tuple = torch.split(padding_mask, split_size_or_sections, dim=1)
#         level_start_index_ = torch.Tensor([0]).to('cuda')
#         import pdb; pdb.set_trace()
        ##############
        
        for _, layer in enumerate(self.layers):
#             for i in range(len(level_start_index)): # add by hj
# # #                 import pdb; pdb.set_trace()
#                 query_list[i] = layer(query_list[i], pos_list[i], reference_points_list[i], spatial_shapes[i].unsqueeze(0), level_start_index_, padding_mask_tuple[i])

            output = layer(output, pos, reference_points, spatial_shapes, level_start_index, padding_mask) # ([8, 3024, 256])
...
class MaskDINOEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.feature_order = feature_order # 'high2low'
        ...
        if self.transformer_num_feature_levels > 1: # proj to 256
            input_proj_list = []
...
#         self.output_convs = output_convs[::-1]
        ...
        # add by hj
#         self.cross_scale_fusion = HybridEncoder(in_channels=[256, 256, 256],
...
#                          depth_mult=1.0,
#                          trt=False,)
#         self.Fake_FPN = Fake_FPN_with_BiFormer(embed_dim=256, 
#             num_heads=8, 
#             num_level_feats=3,
#             topk=16,
#             mlp_dim=1024,
...
#         import pdb; pdb.set_trace()
        # dino's encoder
#         srcs = self.Fake_FPN(srcs)
    
        y, spatial_shapes, level_start_index = self.transformer(srcs, masks, pos)
        
        bs = y.shape[0]
        
        split_size_or_sections = [None] * self.total_num_feature_levels # [None, None, None]
        for i in range(self.total_num_feature_levels):
            if i < self.total_num_feature_levels - 1:
                split_size_or_sections[i] = level_start_index[i + 1] - level_start_index[i]
            else:
                split_size_or_sections[i] = y.shape[1] - level_start_index[i] # [tensor(256, device='cuda:0'), tensor(1024, device='cuda:0'), tensor(4096, device='cuda:0')]
        
        y = torch.split(y, split_size_or_sections, dim=1) # tuple ([2, 256, 256], [2, 1024, 256], [2, 4096, 256])
        ...
        # add by hj
#         out = self.cross_scale_fusion(out)
...
#         return self.mask_features(out[-1]), out[0], multi_scale_features
        return self.mask_features(features['res2'].float()), out[0], multi_scale_features