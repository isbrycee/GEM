...
#             known = [(torch.ones_like(t['labels'])).cuda() for t in targets] # [tensor([1], device='cuda:0'), tensor([1], device='cuda:0')]
#             know_idx = [torch.nonzero(t) for t in known] # [tensor([[0]], device='cuda:0'), tensor([[0]], device='cuda:0')]
#             known_num = [sum(k) for k in known] # [tensor(1, device='cuda:0'), tensor(1, device='cuda:0')]
...
#             unmask_bbox = unmask_label = torch.cat(known) # tensor([1, 1], device='cuda:0')
#             labels = torch.cat([t['labels'] for t in targets]) # tensor([0, 0], device='cuda:0')
#             boxes = torch.cat([t['boxes'] for t in targets]) # tensor([[0.5000, 0.2920, 1.0000, 0.5840],
#                                                              #         [0.3008, 0.0381, 0.3867, 0.0762]], device='cuda:0')
#             batch_idx = torch.cat([torch.full_like(t['labels'].long(), i) for i, t in enumerate(targets)]) # tensor([0, 1], device='cuda:0')
#             # known
#             known_indice = torch.nonzero(unmask_label + unmask_bbox)
#             known_indice = known_indice.view(-1) # tensor([0, 1], device='cuda:0')

#             # noise
#             known_indice = known_indice.repeat(scalar, 1).view(-1) # torch.Size([200]) scalar * bs
#             known_labels = labels.repeat(scalar, 1).view(-1) # torch.Size([200]) scalar * bs
...
#             # noise on the label
#             if noise_scale > 0: # 0.4
#                 p = torch.rand_like(known_labels_expaned.float()) # torch.Size([200]) range from (0, 1)
#                 chosen_indice = torch.nonzero(p < (noise_scale * 0.5)).view(-1)  # half of bbox prob # torch.Size([34])
#                 new_label = torch.randint_like(chosen_indice, 0, self.num_classes)  # randomly put a new one here
#                 known_labels_expaned.scatter_(0, chosen_indice, new_label)
#             if noise_scale > 0:
#                 diff = torch.zeros_like(known_bbox_expand) # torch.Size([200, 4])
#                 diff[:, :2] = known_bbox_expand[:, 2:] / 2
#                 diff[:, 2:] = known_bbox_expand[:, 2:]
#                 known_bbox_expand += torch.mul((torch.rand_like(known_bbox_expand) * 2 - 1.0), # range from [-1, 1]
#                                                diff).cuda() * noise_scale
#                 known_bbox_expand = known_bbox_expand.clamp(min=0.0, max=1.0)

#             m = known_labels_expaned.long().to('cuda')
#             input_label_embed = self.label_enc(m) # ([200, 256])
#             input_bbox_embed = inverse_sigmoid(known_bbox_expand) # ([200, 4])
#             single_pad = int(max(known_num))
#             pad_size = int(single_pad * scalar) # 100
...
#             else:
#                 input_query_label=padding_label.repeat(batch_size, 1, 1) # torch.Size([2, 100, 256])
#                 input_query_bbox = padding_bbox.repeat(batch_size, 1, 1) # torch.Size([2, 100, 4])

#             # map
#             map_known_indice = torch.tensor([]).to('cuda')
#             if len(known_num):
#                 map_known_indice = torch.cat([torch.tensor(range(num)) for num in known_num])  # [1,2, 1,2,3]
#                 map_known_indice = torch.cat([map_known_indice + single_pad * i for i in range(scalar)]).long() # torch.Size([200])
#             if len(known_bid):
#                 input_query_label[(known_bid.long(), map_known_indice)] = input_label_embed # torch.Size([2, 100, 256])
#                 input_query_bbox[(known_bid.long(), map_known_indice)] = input_bbox_embed # torch.Size([2, 100, 4])
...
from .dino_decoder import TransformerDecoder, DeformableTransformerDecoderLayer
from ...utils.utils import MLP, gen_encoder_output_proposals, inverse_sigmoid, QueryProposal
from ...utils import box_ops
...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            # tgt_random = self.query_feat.weight[None].repeat(bs, 1, 1)
            refpoint_embed = self.query_embed.weight[None].repeat(bs, 1, 1)
                
            ##############################
            tgt, proposal_cls_logits = self.query_proposal(
                x
            )
            tgt = tgt.permute(0, 2, 1)
            # tgt = 0.4*tgt_random + 0.6*tgt_sem
    ...
    def forward_prediction_heads(self, output, mask_features, pred_mask=True):
        ...
        if pred_mask:
            ...
            # outputs_mask = torch.einsum("bc,bchw->bhw", decoder_output[:, 75, :], mask_features)[0]
            # mask_cls = outputs_class[0][75]
            # mask_cls = F.softmax(mask_cls, dim=-1)[..., :-1]
            # mask_pred = outputs_mask.sigmoid()
            # semseg = torch.einsum("c,hw->chw", mask_cls, mask_pred)
            # output = semseg.argmax(dim=0).to("cpu")
            # pred = np.array(output, dtype=int) * 255
            # cv2.imwrite("mask_selection.jpg", pred)

            # import matplotlib.pyplot as plt
            # import numpy as np

            # selected_feature_map_batch = mask_features[0]
            # summed_feature_map = np.sum(selected_feature_map_batch.cpu().detach().numpy(), axis=0)
            # fig, ax = plt.subplots(figsize=(96, 96))
            # plt.imshow(summed_feature_map, cmap='viridis')  
            # ax.axis('off')
            # output_file = "summed_feature_map.png"
            ...
            # import cv2
            # # 找到每个特征图上的最大值索引
            # white_image = np.ones((96, 96), dtype=np.uint8) * 255
            # feature_maps = outputs_mask[0]
            # max_idx_list = []
            # # 遍历每个通道
            # for channel in range(100):
            #     # 获取当前通道的特征图
            #     feature_map = feature_maps[channel]

            #     # 找到最大值的索引
            #     max_idx = torch.argmax(feature_map)
            ...
            #     # 在marked_tensor中将对应位置标记为当前通道
            #     white_image[row_idx, col_idx] = 0  
            # cv2.imwrite("/media/hust/disk/haojing/MaskDINO-main-ours/demo/mark.jpg", white_image)
            #######################################
