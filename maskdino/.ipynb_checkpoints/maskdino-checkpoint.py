...
class MaskDINO(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        ###################################
        weight_dict.update({"loss_proposal":80})
        ###################################
    ...
    def forward(self, batched_inputs):
        ...
        ###########################
        # import copy
        # real_images = copy.deepcopy(images)
        # real_images = ImageList.from_tensors(real_images, self.size_divisibility).tensor
        ###########################
        ...
        if self.training:
            ...
            ##############################
            # image_np = images.tensor.cpu().numpy()

            # res = mask_pred_results.unsqueeze(0)
            # res = res.sigmoid()
            ...
            # crf_input = np.transpose(crf_input, (2, 3, 1 ,0))
            ...
            # mask_pred_results = torch.tensor(predict_np)
            ##############################
            ...
            for mask_cls_result, mask_pred_result, mask_box_result, input_per_image, image_size in zip(
                ...
                if self.semantic_on:
                    ...
                    #################################
                    # r_tmp = r.argmax(dim=0).to(torch.device("cpu"))
                    # image_np = real_images.cpu().numpy()

                    # res = r_tmp.unsqueeze(0)
                    # # res = res.sigmoid()
                    # res = (res - res.min()) / (res.max() - res.min() + 1e-8)
                    ...
                    # crf_input = image_np.astype(np.uint8)
                    # crf_input = np.transpose(crf_input, (2, 3, 1 ,0)).squeeze(-1)
                    # crf_input = crf_input.copy(order='C')
                    # # predict_np = np.transpose(predict_np, (1, 2, 0))
                    # # print(crf_input.shape)
                    # # print(predict_np.shape)
                    ...
                    ###########################
                    # processed_results[-1]["sem_seg_1channel"] = r_tmp
                    ###########################
